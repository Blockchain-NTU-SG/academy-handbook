---
title: Acknowledgements
description: Open-source foundations and contributors behind the Academy Handbook.
---

# Acknowledgements

The Blockchain@NTU Academy Handbook builds on open-source documentation
practice and the generosity of communities that make Web3 education easier to
access. This page records those foundations clearly; it does not imply a formal
partnership, sponsorship or endorsement.

## Open-source foundations

The Blockchain@NTU Academy Handbook began from the open-source
[Web3 Internship Handbook](https://github.com/ethpanda-org/Web3-Internship-Handbook)
maintained by the [ETHPanda](https://ethpanda.org/) and [LXDAO](https://lxdao.io/)
communities. We retain and adapt parts of its VuePress + Plume documentation
foundation while developing and maintaining our own curriculum, navigation,
branding and learner experience.

You can explore the original project and its published site here:

- [Web3 Internship Handbook on GitHub](https://github.com/ethpanda-org/Web3-Internship-Handbook)
- [Web3 Internship Handbook website](https://web3intern.xyz/)
- [ETHPanda](https://ethpanda.org/)
- [LXDAO](https://lxdao.io/)

Blockchain@NTU Academy independently maintains the current curriculum,
handbook content and learner experience. The repository’s
[UPSTREAM.md](https://github.com/Blockchain-NTU-SG/academy-handbook/blob/main/UPSTREAM.md)
describes the relationship and retained attribution in more detail.

## Academy contributors

<div class="academy-contributor-links">
  <a class="academy-contributor-link" href="https://x.com/wowwwthemaya" target="_blank" rel="noopener" aria-label="Kai Ze on X">
    <img src="/brand/contributors/kai-ze.jpg" alt="Kai Ze" />
    <span>Kai Ze</span>
  </a>
</div>
